...
(function(window) {
	...
	var TrackingMotionDebugView = window.TrackingMotionDebugView = function() {
		...
		*/
		...
		this.initTexturePlanes = function(videoTexture,renderPasses) {
			...
			this.addTexture(renderPasses['frameDiff'].texture,2,0);
			this.addTexture(renderPasses['direction'].texture,3,0)
			this.addCanvasTexture('direction',0,1)
		}

		this.updateDirection = function(dirx,diry,mag) {
			var canvas = allCanvas['direction'];
	        ...
	        col.g = diry * 0.5 + 0.5;
	        col.b = 1.0;
	     	c.strokeStyle = col.getStyle();
	     	c.lineWidth = c.strokeWidth = 2 + 8 * mag;

	     	c.beginPath();
            c.moveTo(128,128);
            c.lineTo(128 + dirx*100*mag, 128 + diry * 100 * mag);
            c.closePath();
            c.stroke();
	     	c.restore();
