(function() {
  ...
  // var Patch = new tailbone.Model('patches');
  ...
  var Inspector = Sandbox.Inspector = function(options) {
    ...
    if (url.setup) {
      ...
      this.parent.setJSON(Presets.defaults[url.preset].json, makeFullPage);
    } else if (params.defaultJSON) {
      this.parent.setJSON(Presets.defaults[1].json);
    }
    ...
    CreateNavigation: function(inspector) {
      ...
      inspector.$.share = $(inspector.panel.querySelector('.share'))
        ...
        .bind('click', function() {
          ...
          // analytics.trackEvent(AnalyticsController.EVENT_TECH_SHARE, {
          //   label: 'Sharing: ' + json
          // });
          // return;
      ...
      if (param.options) {
        ...
        $(elem.querySelector('select')).change(function(e) {
          ...
          if (_.isFunction(param.onUpdate)) {
            param.onUpdate();
            // analytics.trackEvent(AnalyticsController.EVENT_TECH_PARAM, {
            //   label: node.name + ': ' + this.value
            // });
          }
        });
