...
void main() {
	...
	mediump vec2 off = offset * 2.5 * effectAlpha*drawing;
	mediump vec2 off2 = off * 1.5;
	mediump vec4 blurColor = 
		...
		texture2D(tDiffuse,vUv+vec2(-off.x,off.y)) + 
		texture2D(tDiffuse,vUv+vec2(off.x,off.y)) + 
		texture2D(tDiffuse,vUv+vec2(-off2.x,-off2.y)) + 
		texture2D(tDiffuse,vUv+vec2(off2.x,-off2.y)) + 
		texture2D(tDiffuse,vUv+vec2(-off2.x,off2.y)) + 
		texture2D(tDiffuse,vUv+vec2(off2.x,off2.y));

	color = mix(color,blurColor * 0.12,effectAlpha);
